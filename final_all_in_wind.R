...
C2018 <- read_sav("CX2018_V202102_spss.sav")
C2020 <- read_sav("psfd_c2020_v202304_spss.sav")
C2022 <- read_sav("C2022.sav")
...
# 合併
raw_starting_data <- bind_rows(c2018_sel, c2020_sel, c2022_sel)
...
####初步整理
#受訪西元年統一
raw_starting_data$surv_y[raw_starting_data$surv_y == 109] <- 2020
...
# 對指定欄位應用
starting_data <-raw_starting_data  # 複製原資料
...
# -----------------------------
# 0. 指定其他需差補欄位####
vars_other <- c("int_time", "control_internet")
threshold_tp <- 3.5

# -----------------------------
# 1. 計算原始 talk_parent_raw####
df <- df %>%
  mutate(
    talk_parent_raw = ifelse(
...
# -----------------------------
# 2. 標記需要差補 talk_parent 的案例####
need_impute_idx <- which(is.na(df$talk_parent_raw))

# -----------------------------
# 3. 差補 talk_parent NA 案例的 talk_dad / talk_mom####
if(length(need_impute_idx) > 0){
  df_subset <- df[need_impute_idx, c("talk_dad", "talk_mom")]
  df_subset <- data.frame(lapply(df_subset, as.numeric))
  
  # MICE 差補
  set.seed(123)
  imp_talk <- mice(df_subset, m = 5, method = "pmm", seed = 123)
  df_imputed <- complete(imp_talk, 1)
  
  # 填回原資料
  df[need_impute_idx, c("talk_dad", "talk_mom")] <- df_imputed
}

# -----------------------------
# 4. 計算差補後 talk_parent_imputed & 二值化####
df <- df %>%
  mutate(
    talk_parent_imputed = pmin(talk_dad, talk_mom, na.rm = TRUE),
    talk_parent_bin = as.numeric(talk_parent_imputed <= threshold_tp)
  )

# -----------------------------
# 5. 差補其他欄位 int_time / control_internet####
df_other_subset <- df[, vars_other]
df_other_subset <- data.frame(lapply(df_other_subset, as.numeric))
set.seed(123)
imp_other <- mice(df_other_subset, m = 5, method = "pmm", seed = 123)
df_other_imputed <- complete(imp_other, 1)
df[, vars_other] <- df_other_imputed

# -----------------------------
# 6. 二值化其他欄位####
df <- df %>%
  mutate(
    int_time_bin = as.numeric(int_time > 5.5),
    control_internet_bin = as.numeric(control_internet > 2.5)
  )
...
# -----------------------------
# 8. MICE 多重差補 + Rubin’s Rules####
# 這裡將 int_time/control_internet 差補結果套用即可
fits_mice <- lapply(1:5, function(i){
  dat <- df
...
# -----------------------------
# 9. missForest 單次差補模型####
df_impute_rf <- df[, c(vars_other, "talk_dad", "talk_mom")]
df_impute_rf <- data.frame(lapply(df_impute_rf, as.numeric))
set.seed(123)
df_imputed_rf <- missForest(df_impute_rf)$ximp

df_model_rf <- df
df_model_rf[, c(vars_other, "talk_dad", "talk_mom")] <- df_imputed_rf
df_model_rf <- df_model_rf %>%
  mutate(
    talk_parent_imputed = pmin(talk_dad, talk_mom, na.rm = TRUE),
    talk_parent_bin = as.numeric(talk_parent_imputed <= threshold_tp),
    int_time_bin = as.numeric(int_time > 5.5),
    control_internet_bin = as.numeric(control_internet > 2.5),
    surv_y = as.factor(surv_y)
  )
...
#writecsv####
# #ususalwrite
# write.csv(C2018, "C2018.csv", row.names = FALSE)
# write.csv(C2020, "C2020.csv", row.names = FALSE)
# write.csv(C2022, "C2022.csv", row.names = FALSE)
# #raw_write
# write.csv(raw_starting_data,"raw_starting_data",row.names = FALSE)
# #final_write
# write.csv(starting_data, "starting_data_NA.csv", row.names = FALSE)
# #model_write
# write.csv(df, "df.csv", row.names = FALSE)
#write tsv txt####
#ususalwrite
write.table(C2018, "C2018.txt", row.names = FALSE, sep = "\t")
write.table(C2020, "C2020.txt", row.names = FALSE, sep = "\t")
write.table(C2022, "C2022.txt", row.names = FALSE, sep = "\t")
#raw_write
write.table(raw_starting_data,"raw_starting_data.txt",row.names = FALSE, sep = "\t")
#final_write
write.table(starting_data, "starting_data_NA.txt", row.names = FALSE, sep = "\t")
#model_write
write.table(df, "df.txt", row.names = FALSE, sep = "\t")
