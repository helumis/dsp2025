...
packages <- c("naniar", "tidyverse", "lme4", "lmerTest", "sjPlot","mice", "missForest","broom",
              "psych", "ggplot2", "devtools", "VIM", "dplyr", "grid", "readr","ggalluvial")
...
# (B) missForest 差補版本（只有一組）
df_imputed_rf <- df
df_imputed_rf[, vars_to_impute] <- Imputations_rf$ximp[, vars_to_impute]

# ---- 5. 建立差補下的混合效應模型 ----
formula <- score_happiness ~  sex + age +(surv_y + control_internet +
  intmind_mean + talk_parent + int_time )^2+ (1 | id)
...
# df_imputed_rf$sex <- as.factor(df_imputed_rf$sex)
fit_rf <- lmer(formula, data = df_imputed_rf, REML = FALSE)

# ---- 6. Rubin’s Rules 合併多重差補結果 ----
m <- length(fits_mice)
